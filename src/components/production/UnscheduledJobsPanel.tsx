...
export function UnscheduledJobsPanel({ jobs, allJobs, onStageAdvance, onJobClick }: UnscheduledJobsPanelProps) {
  ...
  const canScheduleHere = isManager || isOperator; // CSR can view-only here
  const canOperate = isManager || isOperator;
	...
	const applyBulkDelete = async () => {
		if (!isManager) {
			toast({ title: 'Not allowed', description: 'Only managers can delete jobs.', variant: 'destructive' });
			return;
		}
				...
				<div className="px-4 pb-4">
					<ScrollArea className="h-48">
						<div className="space-y-2 pr-4">
							{sortedJobs.slice(0, 100).map(job => (
								<div
									key={job.id}
